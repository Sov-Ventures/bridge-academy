...
(function($) {
	...
	qsSlider: function(options) {
		...
		var defaults = {
			...
			PriceBase	: '0.0',		// base price should be as per this calcution (1 CPU Core + 256MB RAM + 50GB HDD)
			PriceCPU	: '.4',		// per month per unit
			PriceRAM	: '1',		// per month per 1024 MB
			PriceHDD 	: '0',		// per month per 10 GB
			
			
			//Maximum Slider values
			MaxCPU	: '150',		// Maximum CPU
			MaxRAM	: '50',		// Maximum RAM //It should be + 2 like if you want maximum RAM 16 GB then make it 18 etc..		
			MaxHDD	: '20',		// Maximum HDD //Its for 500 GB HDD Storage maximum, if want to change it make 100 for 1000 GB, 150 For 1500GB and so on..
		...
		this.each(function(){
			...
			$(function() {
				...
				var presetTips   = new Array;
					presetTips[0] = o.TipsStarter
					presetTips[1] = o.TipsPro
					presetTips[3] = o.TipsScale
				...
				for (var i = 0; i < presetNames.length; i++) {
					...
					$("div#presets").append(
						$('<div>').addClass(presetClass).append(
							$("<button type='button' class='btn btn-sm btn-secondary'>").text(id).on('click', function(e) {
									e.preventDefault();
			...
			var qsSlider = function() {
				...
				var presetspec = {
					//xs:  { cpu: "10",  ram: "0",  hdd: "0"   },
					Starter:   { cpu: "750",  ram: "0",  hdd: "0"   },
					Pro:   { cpu: "0",  ram: "500",  hdd: "0"  }, 
					Scale:   { cpu: "37",  ram: "19", hdd: "5"  }
					//xl:  { cpu: "500",  ram: "100",  hdd: "50" }
				};
				...
				this.updatePrice = function() {
					...
					$("div#QsPrice span.dollerPro").text(price.dollerPro);
					$("div#QsPrice span.dollerEnterprise").text(price.dollerEnterprise);
				...
				var calculatePrice = function() {
					...
					if (hdd == 500) {
						...
						price += ( price10GB * (hdd / 1) ) - (price10GB * 1);
					}
		
					// Check the price, if its a cpanel selected
					if (panel_is_nocp) {
						price = price - ( price * 2* o.discount );
					...
					return {
						...
						"cents":  priceParts[1],
						"price":  price,
						"dollerPro": Math.round(priceParts[0]* 2) +151,
						"dollerEnterprise": Math.round(priceParts[0]* 5) +4
					}
				};
				...
				var checkValueForPreset = function() {
					...
					$("div#presets div.product .btn").removeClass('btn-secondary');        
					...
					for (i in presetspec) {
						...
						if ( presetData.cpu == sliderCPU
							...
							$("div#presets div.product .btn").removeClass('btn-secondary');           
							$("div#presets div."+presetClass+" .btn").addClass('btn-secondary');
						} 
					}
				};
			};
